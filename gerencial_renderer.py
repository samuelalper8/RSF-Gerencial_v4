...
# ══════════════════════════════════════════════════════════════════════════════
# Relatório de Estatísticas — Dashboard executivo em PDF
# ══════════════════════════════════════════════════════════════════════════════

def _draw_kpi_card(page, x: float, y: float, w: float, h: float,
                   valor: str, label: str, sub: str,
                   accent: tuple, fn: dict) -> None:
    """KPI card com borda superior colorida — aspecto de dashboard impresso."""
    # Fundo do card
    _fill(page, x, y, x + w, y + h, WHITE)
    _stroke(page, x, y, x + w, y + h, (0.88, 0.90, 0.93), 0.6)
    # Barra de acento no topo
    _fill(page, x, y, x + w, y + 3.5, accent)

    # Valor central
    vw = _tw(valor, fn["b"], 26)
    _txt(page, x + (w - vw) / 2, y + 36, valor, fn["b"], 26, fill=accent)
    # Label
    lw = _tw(label, fn["b"], 7.5)
    _txt(page, x + (w - lw) / 2, y + 52, label.upper(),
         fn["b"], 7.5, fill=MID)
    # Sublabel
    if sub:
        sw = _tw(sub, fn["r"], 8)
        _txt(page, x + (w - sw) / 2, y + 65, sub, fn["r"], 8, fill=DARK)


def _draw_hbar(page, x: float, y: float, w: float,
               label: str, value: float, max_val: float,
               color: tuple, fn: dict, rank: int = 0) -> float:
    """Barra horizontal com label e valor — retorna y após a barra."""
    bar_h = 14.0
    pct = value / max_val if max_val else 0
    bar_w = max(pct * (w - 160), 2)

    # Rank
    rank_txt = f"#{rank}"
    _txt(page, x, y + 10, rank_txt, fn["b"], 8, fill=color)
    # Label (nome do município)
    _txt(page, x + 22, y + 10, label[:30], fn["r"], 8.5, fill=DARK)
    # Barra
    bar_x = x + 160
    _fill(page, bar_x, y + 2, bar_x + bar_w, y + bar_h, color)
    # Valor R$
    val_fmt = f"R$ {value:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    _txt(page, bar_x + bar_w + 6, y + 10, val_fmt,
         fn["b"], 7.5, fill=color)
    return y + bar_h + 5


def _draw_status_bar(page, x: float, y: float, w: float,
                     label: str, count: int, total: int,
                     color: tuple, fn: dict) -> float:
    """Barra de status horizontal compacta com contagem."""
    pct = count / total if total else 0
    _txt(page, x, y + 9, label, fn["r"], 8.5, fill=DARK)
    _txt(page, x + 120, y + 9, str(count), fn["b"], 9, fill=color)
    bar_x = x + 145
    bar_w = w - 155
    _fill(page, bar_x, y + 3, bar_x + bar_w, y + 12, (0.92, 0.94, 0.96))
    _fill(page, bar_x, y + 3, bar_x + max(pct * bar_w, 1), y + 12, color)
    return y + 20


def render_estatisticas_pdf(
    stats: dict,
    ger_dir: Path,
    logo: Path | None = None,
    ts: str = "",
    ...
    """
    Gera PDF executivo com painel de estatísticas — design profissional
    para impressão em A4-paisagem.

    Página 1: KPIs + Top Devedores + CND + Omissões por declaração
    Página 2+: Tabela detalhada por município
    """
    if fitz is None:
        raise RuntimeError("PyMuPDF não instalado.")
    if not ts:
    ...
    ctx = _new_ctx("PAINEL EXECUTIVO · ESTATÍSTICAS", logo)
    fn  = ctx.fn
    page = ctx.page
    ...
    kpis = [
        (str(n_total), "Municípios Analisados", "", (0.30, 0.45, 0.65)),
        (str(totais.get("muns_dev", 0)), "Com Devedor",
         brl(totais.get("v_devedor", 0)), RED),
        (str(totais.get("muns_maed", 0)), "Com MAED",
         brl(totais.get("v_maed", 0)), AMBER),
        (str(totais.get("muns_omiss", 0)), "Com Omissões",
         f"{totais.get('n_omiss', 0)} itens", SKY),
        (str(totais.get("muns_pf", 0)), "Processo Fiscal",
         f"{totais.get('n_pf', 0)} itens", (0.55, 0.36, 0.86)),
    ]

    for i, (val, lbl, sub, col) in enumerate(kpis):
        kx = _ML + i * (kpi_w + 14)
        _draw_kpi_card(page, kx, kpi_y, kpi_w, kpi_h,
                       val, lbl, sub, col, fn)

    ctx.y = kpi_y + kpi_h + 20

    # ── Divisão em 3 colunas: Top Dev | CND | Omissões por Decl ──────────
    col_w = (_CW - 2 * 20) / 3
    col_x = [_ML, _ML + col_w + 20, _ML + 2 * (col_w + 20)]
    sec_y = ctx.y

    # ── Coluna 1: Top Devedores ──────────────────────────────────────────
    _fill(page, col_x[0], sec_y, col_x[0] + col_w, sec_y + 22,
          (0.94, 0.95, 0.97))
    _txt(page, col_x[0] + 8, sec_y + 15,
         "TOP DEVEDORES (VALOR CONSOLIDADO)", fn["b"], 8, fill=NAVY)
    bar_y = sec_y + 30

    if top:
        max_v = top[0]["valor"] if top else 1
        rank_colors = [RED, RED, AMBER, AMBER, (0.75, 0.60, 0.10),
                       (0.75, 0.60, 0.10), MID, MID, MID, MID]
        for i, r in enumerate(top[:8]):
            col_bar = rank_colors[i] if i < len(rank_colors) else MID
            bar_y = _draw_hbar(page, col_x[0] + 4, bar_y, col_w - 8,
                               r["nome"], r["valor"], max_v,
                               col_bar, fn, rank=i + 1)
    else:
        _txt(page, col_x[0] + 8, bar_y + 10,
             "Nenhum devedor encontrado.", fn["r"], 9, fill=MID)

    # ── Coluna 2: CND Status ─────────────────────────────────────────────
    _fill(page, col_x[1], sec_y, col_x[1] + col_w, sec_y + 22,
          (0.94, 0.95, 0.97))
    _txt(page, col_x[1] + 8, sec_y + 15,
         "STATUS DAS CNDs", fn["b"], 8, fill=NAVY)
    cnd_y = sec_y + 32
    total_cnd = sum(cnd.values()) or 1
    cnd_rows = [
        (cnd.get("vencidas", 0), "Vencidas",       RED),
        (cnd.get("urgente", 0),  "Vencem em 30d",  AMBER),
        (cnd.get("atencao", 0),  "Vencem em 90d",  (0.75, 0.60, 0.10)),
        (cnd.get("ok", 0),       "Válidas (+90d)", GREEN),
    ]
    for cnt, lbl, col_c in cnd_rows:
        cnd_y = _draw_status_bar(page, col_x[1] + 4, cnd_y, col_w - 8,
                                 lbl, cnt, total_cnd, col_c, fn)

    # Resumo textual
    cnd_y += 10
    cnd_total = sum(cnd.values())
    if cnd_total:
        _txt(page, col_x[1] + 8, cnd_y,
             f"Total de CNDs analisadas: {cnd_total}", fn["r"], 8, fill=MID)

    # ── Coluna 3: Omissões por Declaração ─────────────────────────────────
    _fill(page, col_x[2], sec_y, col_x[2] + col_w, sec_y + 22,
          (0.94, 0.95, 0.97))
    _txt(page, col_x[2] + 8, sec_y + 15,
         "OMISSÕES POR DECLARAÇÃO", fn["b"], 8, fill=NAVY)
    decl_y = sec_y + 32
    total_decl = sum(bd.values()) or 1
    for decl, cnt in list(bd.items())[:10]:
        dcol = _DECL_COLORS.get(decl, MID)
        decl_y = _draw_status_bar(page, col_x[2] + 4, decl_y, col_w - 8,
                                  decl, cnt, total_decl, dcol, fn)

    if not bd:
        _txt(page, col_x[2] + 8, decl_y + 10,
             "Nenhuma omissão encontrada.", fn["r"], 9, fill=MID)

    # ── Página 2+: Tabela detalhada por município ────────────────────────
    if por_m:
        ctx.page = ctx.doc.new_page(width=_W, height=_H)
        ctx.pnum += 1
        ctx.y = _draw_hdr(ctx.page, fn,
                          "RELATÓRIO GERENCIAL  ·  DETALHAMENTO POR MUNICÍPIO",
                          ctx.info, logo, ctx.pnum)

        # Cabeçalho da tabela
        _tbl_hdr_cols = [
            (_ML + 4,   "MUNICÍPIO",  200),
            (_ML + 200, "DEV.",        40),
            (_ML + 240, "VALOR DEV.",  95),
            (_ML + 335, "MAED",        40),
            (_ML + 375, "VALOR MAED",  95),
            (_ML + 470, "OMISSÕES",    55),
            (_ML + 525, "P.F.",        35),
            (_ML + 565, "CND (DIAS)",  65),
            (_ML + 635, "SITUAÇÃO",    90),
        ...
        hdr_h = 18
        _fill(ctx.page, _ML, ctx.y - 2, _W - _MR, ctx.y + hdr_h, NAVY)
        for cx, lbl, _ in _tbl_hdr_cols:
            _txt(ctx.page, cx, ctx.y + 12, lbl, fn["b"], 7, fill=WHITE_DIM)
        ctx.y += hdr_h + 4
        ...
        for i, r in enumerate(por_m):
            ...
            # Fundo alternado
            if i % 2 == 0:
                _fill(ctx.page, _ML, row_y - 2, _W - _MR, row_y + 16, GRAY_ROW)

            # Status dot
            has_dev = r["n_dev"] > 0
            has_issues = r["n_maed"] > 0 or r["n_omiss"] > 0
            dot_col = RED if has_dev else AMBER if has_issues else GREEN
            page_cur = ctx.page
            page_cur.draw_circle(fitz.Point(_ML + 8, row_y + 7),
                                 3, color=None, fill=dot_col)

            # Nome
            nome = r["nome"][:32] + ("…" if len(r["nome"]) > 32 else "")
            _txt(ctx.page, _ML + 16, row_y + 11, nome, fn["r"], 8.5, fill=DARK)

            # n_dev
            nd = str(r["n_dev"]) if r["n_dev"] else "—"
            nd_col = RED if r["n_dev"] else MID
            _txt(ctx.page, _ML + 208, row_y + 11, nd, fn["b"], 8.5, fill=nd_col)

            # v_dev
            vd = brl(r["v_dev"]) if r["v_dev"] > 0 else "—"
            _txt(ctx.page, _ML + 242, row_y + 11, vd,
                 fn["b"] if r["v_dev"] > 0 else fn["r"], 7.5,
                 fill=RED if r["v_dev"] > 0 else MID)

            # n_maed
            nm = str(r["n_maed"]) if r["n_maed"] else "—"
            _txt(ctx.page, _ML + 345, row_y + 11, nm,
                 fn["b"], 8.5, fill=AMBER if r["n_maed"] else MID)

            # v_maed
            vm = brl(r["v_maed"]) if r["v_maed"] > 0 else "—"
            _txt(ctx.page, _ML + 377, row_y + 11, vm,
                 fn["b"] if r["v_maed"] > 0 else fn["r"], 7.5,
                 fill=AMBER if r["v_maed"] > 0 else MID)

            # omiss
            no = str(r["n_omiss"]) if r["n_omiss"] else "—"
            _txt(ctx.page, _ML + 480, row_y + 11, no,
                 fn["r"], 8.5, fill=SKY if r["n_omiss"] else MID)

            # pf
            np_ = str(r["n_pf"]) if r["n_pf"] else "—"
            _txt(ctx.page, _ML + 535, row_y + 11, np_,
                 fn["r"], 8.5, fill=(0.55, 0.36, 0.86) if r["n_pf"] else MID)

            # CND dias
            cd = r.get("cnd_dias")
            if cd is not None:
                if cd < 0:
                    cnd_txt, cnd_col = f"{cd}d", RED
                elif cd <= 30:
                    cnd_txt, cnd_col = f"{cd}d", AMBER
                elif cd <= 90:
                    cnd_txt, cnd_col = f"{cd}d", (0.75, 0.60, 0.10)
                else:
                    cnd_txt, cnd_col = f"{cd}d", GREEN
                _txt(ctx.page, _ML + 580, row_y + 11, cnd_txt,
                     fn["b"], 8.5, fill=cnd_col)
            else:
                _txt(ctx.page, _ML + 580, row_y + 11, "—",
                     fn["r"], 8.5, fill=MID)

            # Situação (badge)
            total_issues = r["n_dev"] + r["n_maed"] + r["n_omiss"] + r["n_pf"]
            if total_issues == 0:
                _badge(ctx.page, _ML + 640, row_y + 11,
                       "REGULAR", GREEN, WHITE, fs=7, fn=fn["b"])
            elif r["n_dev"]:
                _badge(ctx.page, _ML + 640, row_y + 11,
                       "PENDÊNCIAS", RED, WHITE, fs=7, fn=fn["b"])
            else:
                _badge(ctx.page, _ML + 640, row_y + 11,
                       "ATENÇÃO", AMBER, WHITE, fs=7, fn=fn["b"])

            ctx.y += 20

        # Linha separadora final
        _hline(ctx.page, _ML, ctx.y, _W - _MR, GRAY_SEP, 0.7)
        ctx.y += 10

        # Rodapé com totais
        ctx.need(28)
        _fill(ctx.page, _ML, ctx.y, _W - _MR, ctx.y + 22, NAVY)
        _txt(ctx.page, _ML + 16, ctx.y + 14, "TOTAL CONSOLIDADO",
             fn["b"], 9, fill=WHITE)
        _txt(ctx.page, _ML + 242, ctx.y + 14, brl(totais.get("v_devedor", 0)),
             fn["b"], 8, fill=(1.0, 0.55, 0.55))
        _txt(ctx.page, _ML + 377, ctx.y + 14, brl(totais.get("v_maed", 0)),
             fn["b"], 8, fill=AMBER_LT)
        _txt(ctx.page, _ML + 480, ctx.y + 14, str(totais.get("n_omiss", 0)),
             fn["b"], 8, fill=(0.60, 0.85, 1.0))
        _txt(ctx.page, _ML + 535, ctx.y + 14, str(totais.get("n_pf", 0)),
             fn["b"], 8, fill=(0.80, 0.70, 1.0))

    out = ger_dir / f"ESTATISTICAS_{ts[:12] or 'R'}.pdf"
    ctx.doc.save(str(out))
    ctx.doc.close()
    return out
...
# ══════════════════════════════════════════════════════════════════════════════
# Relatório Consolidado — merge de TODOS os gerenciais num único PDF
# ══════════════════════════════════════════════════════════════════════════════

def render_consolidado_completo(
    ger_dir: Path,
    logo: Path | None = None,
    ts: str = "",
) -> Path:
    """
    Gera um PDF consolidado fundindo todos os gerenciais de ger_dir
    na seguinte ordem: Estatísticas → Devedor → MAED → Omissão →
    Processo Fiscal → Validade CND → Unificado.

    Cada relatório é precedido por uma capa-separadora de seção
    para navegação fácil em impressão.
    """
    if fitz is None:
        raise RuntimeError("PyMuPDF não instalado.")
    if not ts:
    ...
    # Ordem canônica de merge (prefix → label para capa)
    sections = [
        ("ESTATISTICAS_", "PAINEL EXECUTIVO · ESTATÍSTICAS", (0.30, 0.45, 0.65)),
        ("DEVEDO_",       "RELATÓRIO DEVEDOR",               RED),
        ("MAEDS__",       "RELATÓRIO MAED",                  AMBER),
        ("OMISSO_",       "RELATÓRIO DE OMISSÕES",           SKY),
        ("PROCES_",       "RELATÓRIO PROCESSO FISCAL",       (0.55, 0.36, 0.86)),
        ("VALIDA_",       "VALIDADE DE CND",                 GREEN),
        ("UNIFICADO_",    "RELATÓRIO UNIFICADO POR MUNICÍPIO", NAVY_LT),
    ]

    def _insert_section_cover(label: str, accent: tuple, section_num: int) -> None:
        """Cria uma página-capa para cada seção do consolidado."""
        cover = merged.new_page(width=_W, height=_H)
        # Fundo navy
        _fill(cover, 0, 0, _W, _H, NAVY)
        # Faixa central decorativa
        cy = _H / 2 - 40
        _fill(cover, 0, cy, _W, cy + 80, (0.06, 0.16, 0.28))
        _fill(cover, 0, cy, 6, cy + 80, accent)
        _fill(cover, 0, cy + 78, _W, cy + 80, accent)

        # Logo
        if logo and logo.exists():
            try:
                cover.insert_image(_R(_W / 2 - 45, cy - 65, _W / 2 + 45, cy - 10),
                                   filename=str(logo))
            except Exception:
                pass

        # Número da seção
        sec_lbl = f"SEÇÃO {section_num}"
        sw = _tw(sec_lbl, fn["r"], 10)
        _txt(cover, (_W - sw) / 2, cy + 28, sec_lbl,
             fn["r"], 10, fill=accent)

        # Título da seção
        tw = _tw(label, fn["b"], 22)
        _txt(cover, (_W - tw) / 2, cy + 55, label,
             fn["b"], 22, fill=WHITE)

        # Rodapé
        footer = f"ConPrev  ·  Análise de Restrições  ·  {time.strftime('%d/%m/%Y')}"
        fw = _tw(footer, fn["r"], 8)
        _txt(cover, (_W - fw) / 2, _H - 30, footer,
             fn["r"], 8, fill=(0.40, 0.52, 0.65))

    section_num = 0
    for prefix, label, accent in sections:
        # Procura arquivo que bate com o prefixo
        matches = sorted(ger_dir.glob(f"{prefix}*.pdf"))
        if not matches:
            continue
        pdf_path = matches[0]

        section_num += 1
        _insert_section_cover(label, accent, section_num)

        # Insere todas as páginas do relatório
        try:
    ...
    # Caso nenhum arquivo encontrado
    if section_num == 0:
        cover = merged.new_page(width=_W, height=_H)
        _fill(cover, 0, 0, _W, _H, NAVY)
        _txt(cover, _ML, _H / 2,
             "Nenhum relatório gerencial encontrado.", fn["r"], 14, fill=MID)

    out = ger_dir / f"CONSOLIDADO_COMPLETO_{ts[:12] or 'R'}.pdf"
    merged.save(str(out))
    merged.close()
    return out
