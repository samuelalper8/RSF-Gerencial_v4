...
# 1. Trava de Caminho Absoluto: Garante que o arquivo seja salvo na mesma pasta do app.py
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ARQUIVO_CLIENTES = os.path.join(BASE_DIR, "clientes_base.json")

# 2. Lote Blindado de Clientes
DADOS_LOTE = """
...
def _gerar_lote_padrao() -> list[dict]:
    """Lê o DADOS_LOTE acima e converte para a estrutura correta."""
    clientes_iniciais = []
    for linha in DADOS_LOTE.strip().split("\n"):
        linha = linha.strip()
        if not linha or "MUNICÍPIO E VINCULADOS" in linha: continue
        ...
        for nat in ['Autarquia', 'Câmara', 'Conselho Esc', 'Município', 'RPPS']:
        ...
        clientes_iniciais.append({"cnpj": cnpj_fmt, "nome": f"{nome} ({uf})"})
    return clientes_iniciais

def carregar_clientes() -> list[dict]:
    """Lê o arquivo JSON. Se estiver vazio ou só tiver testes, auto-injeta os 147."""
    clientes_atuais = []
    
    # 3. Tenta ler o arquivo local
    if os.path.exists(ARQUIVO_CLIENTES):
        try:
            with open(ARQUIVO_CLIENTES, "r", encoding="utf-8") as f:
                clientes_atuais = json.load(f)
        except json.JSONDecodeError:
            clientes_atuais = []
            
    # 4. AUTO-INJEÇÃO: Se tiver menos de 5 clientes (como o seu "TESTE"), injeta a base real.
    if len(clientes_atuais) < 5:
        lote_padrao = _gerar_lote_padrao()
        cnpjs_existentes = {c['cnpj'] for c in clientes_atuais}
        for novo in lote_padrao:
            if novo['cnpj'] not in cnpjs_existentes:
                clientes_atuais.append(novo)
        # Salva o arquivo atualizado no diretório correto
        salvar_clientes(clientes_atuais)
