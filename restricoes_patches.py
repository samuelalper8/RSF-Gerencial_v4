...
import re, shutil, traceback, unicodedata
...
def patch_analisar_restricoes() -> None:
    ...
    def _patched(base_dir, municipios_escolhidos, incluir_subpastas, out_root, log_cb):
        ...
        try:
            ...
            #         gerados por _orig (layout padrão) são preservados.
            ger_tmp = ger_dir.parent / (ger_dir.name + "_tmp")
            ger_tmp.mkdir(parents=True, exist_ok=True)
            try:
                from gerencial_renderer import (
                    render_all_gerenciais, render_unificado_municipios,
                    render_estatisticas_pdf, render_consolidado_completo,
                )
                ...
                if _last_stats:
                    log_cb("📊 Gerando painel executivo (PDF)…")
                ...
                for old in ger_dir.glob("*.pdf"):
                    try:
                        ...
                        pass
                for new_pdf in ger_tmp.glob("*.pdf"):
                    shutil.move(str(new_pdf), str(ger_dir / new_pdf.name))

                log_cb("✅ Layout visual e estatísticas gerados.")

            except Exception as exc:
                # FIX 3: loga o traceback completo para diagnóstico remoto.
                #         Os PDFs originais de _orig são preservados em ger_dir
                #         (graças ao swap atômico: só apagamos após sucesso).
                tb = traceback.format_exc()
                log_cb(
                    f"[AVISO] Renderer visual falhou — layout padrão mantido.\n"
                    f"Erro: {exc}\n"
                    f"Traceback:\n{tb}"
                )
            finally:
                # Limpa o diretório temporário independentemente do resultado
                shutil.rmtree(ger_tmp, ignore_errors=True)

        except Exception as exc:
            tb = traceback.format_exc()
            log_cb(
                f"[AVISO] Falha no pós-processamento (estatísticas/renderer).\n"
                f"Erro: {exc}\n"
                f"Traceback:\n{tb}"
            )
        return result
