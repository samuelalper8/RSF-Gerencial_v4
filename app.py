...
def _sync_checkboxes(uf: str, state: bool) -> None:
    """Sincroniza session_state dos widgets checkbox com o dict de seleção.

    Sem isso, st.checkbox(..., key=...) ignora o parâmetro `value=`
    após a primeira renderização, porque a source of truth passa a ser
    st.session_state[key] — e não o dict manual.
    """
    ...
    if ss.analysis_done and ss.result_zip_bytes:
        ...
        <div style="background:linear-gradient(135deg,rgba(42,156,107,.1),rgba(42,156,107,.05));
          ...
          <div style="font-size:12.5px;color:#7a95ad;line-height:1.6">
            ...
            <b style="color:#b0c4d8">8 relatórios gerenciais</b>:
            Devedor, MAED, Omissões, Processo Fiscal, Validade CND,
            Relatório Unificado, <b style="color:#F29F05">Painel Executivo de Estatísticas</b> e
            <b style="color:#2d8fd4">Relatório Consolidado Completo</b> (todos num só PDF com capas de seção).
          </div>
        </div>""", unsafe_allow_html=True)
