...
try:
    ...
    def listar_clientes(): return []
    def cadastrar_cliente(c, n): return False
    def remover_cliente(c): return False

try:
    from relatorio_html import gerar_html_executivo
except ImportError:
    def gerar_html_executivo(stats, municipios): return "<h1>Erro: HTML Module não encontrado</h1>"
...
[data-baseweb="select"]>div{background:rgba(255,255,255,.05)!important;
  border:1px solid var(--border2)!important;border-radius:var(--radius)!important;color:var(--text)!important}
[data-baseweb="select"]>div:focus-within{border-color:var(--sky)!important;box-shadow:0 0 0 3px rgba(45,143,212,.18)!important}
...
.stat-label{font-size:10.5px;font-weight:600;color:#7a95ad;text-transform:uppercase;letter-spacing:1px;margin-top:4px}
...
def _sync_checkboxes(uf: str, state: bool) -> None:
    muns = MUNICIPIOS_POR_UF.get(uf, [])
    ss[f"mun_sel_{uf}"] = {m: state for m in muns}
    for m in muns:
        ss[f"cb_{uf}_{m}"] = state
...
def render_stats(stats: dict) -> None:
    ...
    with c2: _stat_card(str(totais.get("muns_dev",0)),  "Com DEVEDOR", sub=_brl_fmt(totais.get("v_devedor",0)), color="#d63b3b")
    with c3: _stat_card(str(totais.get("muns_maed",0)), "Com MAED", sub=_brl_fmt(totais.get("v_maed",0)), color="#e8a020")
    with c4: _stat_card(str(totais.get("muns_omiss",0)), "Com Omissões", sub=f"{totais.get('n_omiss',0)} itens", color="#2d8fd4")
    with c5: _stat_card(str(totais.get("muns_pf",0)), "Processo Fiscal", sub=f"{totais.get('n_pf',0)} itens", color="#8b5cf6")
    ...
    with col_cnd:
        st.markdown(
            ...
            'text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">🔐 Status das CNDs</p>', unsafe_allow_html=True)
        cnd_rows = [
        ...
        for n, lbl, col in cnd_rows:
            ...
            st.markdown(
                ...
                f'<span style="color:#b0c4d8">{lbl}</span><span style="color:{col};font-weight:700">{n}</span></div>'
    ...
    with col_top:
        st.markdown(
            '<p style="font-size:11px;font-weight:700;color:#7a95ad;'
            'text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">🏆 Top Devedores</p>', unsafe_allow_html=True)
        if top:
            max_val = top[0]["valor"] if top else 1
            for i, r in enumerate(top[:8]):
                pct = r["valor"] / max_val * 100 if max_val else 0
                rank_col = ["#d63b3b","#d63b3b","#F29F05","#F29F05","#e8a020","#e8a020","#7a95ad","#7a95ad"][i]
                ...
                st.markdown(
                    ...
                    f'<span style="color:#b0c4d8"><span style="color:{rank_col};font-weight:700;margin-right:5px">#{i+1}</span>{nome_short}</span>'
                    f'<span style="color:{rank_col};font-weight:700;font-family:\'IBM Plex Mono\',monospace;font-size:10.5px">{_brl_fmt(r["valor"])}</span></div>'
                    f'<div style="background:rgba(255,255,255,.06);border-radius:3px;height:4px">'
                    f'<div style="background:{rank_col};width:{pct:.0f}%;height:4px;border-radius:3px;opacity:.75"></div>'
                    f'</div></div>', unsafe_allow_html=True)
        else:
            st.markdown('<p style="font-size:12px;color:#7a95ad">Nenhum devedor encontrado.</p>', unsafe_allow_html=True)

    with col_bd:
        st.markdown(
            '<p style="font-size:11px;font-weight:700;color:#7a95ad;'
            'text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">📋 Omissões</p>', unsafe_allow_html=True)
        if bd:
            total_bd = sum(bd.values()) or 1
            decl_colors = {"DCTF":"#1a6faf","DCTFWEB":"#2d8fd4","SISOBRA":"#2a9c6b","GFIP":"#8b5cf6","ECF":"#10b981","PGFN":"#d63b3b"}
            for decl, cnt in list(bd.items())[:8]:
                ...
                st.markdown(
                    f'<div style="margin-bottom:5px">'
                    f'<div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:2px">'
                    f'<span style="color:#b0c4d8">{decl}</span><span style="color:{col_d};font-weight:700">{cnt}</span></div>'
                    f'<div style="background:rgba(255,255,255,.06);border-radius:3px;height:4px">'
...
def render_app():
    ...
    # PAINEL ESQUERDO — Seleção multi-UF
    with col_left:
        ...
        total_sel = sum(sum(ss.get(f"mun_sel_{uf}",{}).values()) for uf in _UFS)
        ...
        st.markdown(
            ...
            f'<span style="font-size:16px;font-weight:800;color:#F29F05">{total_sel}</span>'
        ...
        with gb1:
            if st.button("✅ Todos (GO+TO+MS)", key="all_global", use_container_width=True):
                for uf in _UFS: _sync_checkboxes(uf, True)
                st.rerun()
        with gb2:
            if st.button("✗ Limpar tudo", key="clr_global", use_container_width=True):
                for uf in _UFS: _sync_checkboxes(uf, False)
                st.rerun()

        tab_go, tab_to, tab_ms = st.tabs(["🟡  GO", "🔵  TO", "🟢  MS"])
            ...
            with tab_map[uf]:
                ...
                with b1:
                    if st.button(f"✅ Todos ({uf})", key=f"all_{uf}", use_container_width=True):
                        _sync_checkboxes(uf, True); st.rerun()
                with b2:
    ...
    with col_right:
        _section("Upload dos Relatórios", "📤", accent="#2d8fd4")
        uploaded = st.file_uploader(
            "PDFs ou ZIP com os relatórios", type=["pdf","zip"],
            accept_multiple_files=True, label_visibility="collapsed")
        if uploaded:
            st.success(f"📎 Recebido: {len(uploaded)} arquivo(s)")
    ...
    # Botão analisar
    st.markdown('<div style="height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.07),transparent);margin:20px 0 16px"></div>', unsafe_allow_html=True)

    selected_muns = [m for uf in _UFS for m in MUNICIPIOS_POR_UF.get(uf,[]) if ss.get(f"mun_sel_{uf}",{}).get(m,False)]
    ...
    if st.button(btn_label, type="primary", use_container_width=True, disabled=not can_run):
        ...
        with st.spinner("Processando PDFs…"):
            result = run_analysis(uploaded, selected_muns, log_ph, logo_bytes=logo_bytes, filtro_decl=filtro_decl or None)
        if result:
            ss.result_zip_bytes, ss.result_file_count, ss.last_stats, ss.last_ts = result
            ss.analysis_done = True; st.rerun()

    # ── Resultado + Estatísticas (Com inclusão do Botão HTML) ──
    if ss.analysis_done and ss.result_zip_bytes:
        ...
        <div style="background:linear-gradient(135deg,rgba(42,156,107,.1),rgba(42,156,107,.05));
            ...
            border-radius:12px;padding:18px 22px;margin-top:14px; margin-bottom: 20px;">
        ...
        with col_down1:
            st.download_button(
                label=f"📦 Baixar Arquivos (ZIP)",
                data=ss.result_zip_bytes,
                file_name=f"ConPrev_Restricoes_{ts}.zip",
                mime="application/zip", use_container_width=True)
                
        with col_down2:
            if ss.last_stats and 'municipios' in ss.last_stats:
                html_str = gerar_html_executivo(ss.last_stats, ss.last_stats['municipios'])
                st.download_button(
                    label="📊 Baixar Relatório Executivo (HTML)",
                    data=html_str.encode("utf-8"),
                    file_name=f"Relatorio_Executivo_Restricoes_{time.strftime('%Y%m%d')}.html",
                    mime="text/html",
                    use_container_width=True
                )
            else:
                st.info("Estatísticas insuficientes para o HTML.")
        ...
        # Renderização do painel original
        if ss.last_stats:
...
if not ss.authenticated:
    ...
    if _DASHBOARD_OK:
        ...
        st.error("Módulo de sócios não disponível.")
else:
    render_app()
